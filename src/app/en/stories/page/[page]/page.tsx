...
export async function generateStaticParams() {
  const allPosts = getAllPosts('stories', 'en')
...
export default async function StoriesPage({ params }: StoriesPageProps) {
  ...
  const { posts: stories, totalPages, currentPage, hasNextPage, hasPreviousPage } = getPaginatedPosts('stories', pageNumber, 6, 'en')
        ...
        <div className="max-w-6xl mx-auto px-12 md:px-16 lg:px-20 xl:px-24">
          {stories.length === 0 ? (
            <div className="text-center py-24">
              <p className="text-xl text-gray-600 mb-4">{t.stories.noStories}</p>
            ...
            <div className="grid gap-16 md:gap-24">
              {stories.map((story) => (
                <article key={story.metadata.slug} className="group">
                  <Link href={`/stories/en/${story.metadata.slug}`}>
                      ...
                      <div className="space-y-6">
                        <div className="space-y-4">
                          <div className="text-xs uppercase tracking-ultra-wide text-gray-500 font-bold">
                            {story.metadata.date && new Date(story.metadata.date).toLocaleDateString('en-US', {
                              year: 'numeric',
                              month: 'long',
                              day: 'numeric'
                            })}
                          </div>
                          <h2 className="text-xl md:text-2xl font-bold text-gray-900 leading-tight group-hover:text-gray-700 transition-colors tracking-tight">
                        ...
                        <div className="pt-4">
                          <span className="inline-flex items-center text-sm font-bold text-gray-900 group-hover:text-gray-700 transition-colors tracking-wider uppercase">
                            Read Story
